...
export function OpenSpecCommand({ store, args }: OpenSpecCommandProps) {
  ...
  useEffect(() => {
    ...
    if (subcommand === 'bind') {
      // th openspec bind <task-id> <change-id> [--capability <capability>]
      ...
      if (!taskId || !changeId) {
        setError('Usage: th openspec bind <task-id> <change-id> [--capability <capability>]');
        return;
      }
  ...
  th openspec scan [--path <path>]   Scan for OpenSpec project
  th openspec list [--path <path>]   List changes and capabilities
  th openspec bind <task-id> <change-id> [--capability <cap>]  Bind task to change
`);
  }, [store, args]);
